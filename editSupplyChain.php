...
$stid = oci_parse($conn, "SELECT * FROM SUPPLYCHAINS INNER JOIN KEYEVENTS ON SUPPLYCHAINS.EVENTID = KEYEVENTS.EVENTID WHERE SUPPLYID = :SupplyID");
oci_bind_by_name($stid, ":SupplyID", $SupplyID);
oci_execute($stid);
...
<head>
  ...
  <title><?php echo $applicationName; ?> - Edit Supply Chain</title>
...
<body>
  ...
  <div class="container">
    ...
    <h1 class="display-1">Edit Supply Chain</h1><br>
    ...
    <?php while ($row = oci_fetch_array($stid, OCI_ASSOC + OCI_RETURN_NULLS)) { ?>
      ...
      <form action="processor/supplychainProcessor.php" method="post">
        ...
        <div class="form-group">
          ...
          ​<textarea id="SupplyDescription" name="SupplyDescription" class="form-control" rows="5" cols="70" required="true" maxlength="255"><?php echo $row['SUPPLYDESCRIPTION']; ?></textarea>
        </div>

        <div class="form-group">
          <label for="TariffDetails">Tariff Details:</label>
          ​<textarea id="TariffDetails" name="TariffDetails" class="form-control" rows="5" cols="70" required="true" maxlength="255"><?php echo $row['TARIFFDETAILS']; ?></textarea>
        </div>
